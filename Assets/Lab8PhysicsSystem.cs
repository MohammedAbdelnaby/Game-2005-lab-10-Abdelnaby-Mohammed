...
public class Lab8PhysicsSystem : MonoBehaviour
{
    ...
    {
        float X = Mathf.Max(Box.GetMin().x, Mathf.Min(sphere.transform.position.x, Box.GetMax().x));
        float Y = Mathf.Max(Box.GetMin().y, Mathf.Min(sphere.transform.position.y, Box.GetMax().y));
        float Z = Mathf.Max(Box.GetMin().z, Mathf.Min(sphere.transform.position.z, Box.GetMax().z));
    ...
    void ApplyMinimumTranlationVector(Lab8PhysicsObjects object1, Lab8PhysicsObjects object2, Vector3 miniumTranslationVectorAtoB, Vector3 normal, Vector3 contactPoint)
    {
        ...
        Debug.Log(ScalerB);
        object1.transform.position += TranslationVectorA;
        object2.transform.position += TranslationVectorB;
    }
    ...
    void KinematicsCollisionResponse(CollisionInfo info)
    {
        ...
        else if (!objectB.LockPosition && objectB.LockPosition)
        {
